...
class WindowsService(win32serviceutil.ServiceFramework):
    ...
    def __init__(self, *args):
        ...
        src_path = Path(self._dir_path_ + "\\common")
        sys.path.append(src_path)

        from standard import Standard

        log_file = Path(self._src_path_ + Standard.gDirLog + "service.log")
        print(log_file)
    ...
    def main(self):
        print("main")
        try:
            src_path = Path(self._dir_path_ + "\\scheduler")
            sys.path.append(src_path)
...
#
# Usage: "WindowsService.py [options] install|update|remove|start [...]|stop|restart [...]|debug [...]"
