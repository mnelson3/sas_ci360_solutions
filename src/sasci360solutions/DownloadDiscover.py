...
from custom import BuildEmailData
from custom import root_path
from log import Log
from standard import Standard, CleanData, CreateData, DownloadEntity
...
class DownloadDiscover:
	...
	def run(self):
		try:
			...
			logger.info(msg="getDownloadUrl request duration: " + str(get_urls_duration) + " seconds")
			...
			if "error" in json_data:
				logger.error("Error: " + json_data["error"] + " - " + json_data["message"])
				sys.exit()
